...
const sendTelegramMessage = async (messages, telegramId) => {
  return true;
};
...
const checkUnique = async (conditions) =>{
  let i = 0;
  let where = '';
...
const findOne = async (conditions, fields = '*') => {
  const err = {
    status: httpStatus.UNAUTHORIZED,
    isPublic: true,
  };
...
const updateOne = async (conditions, adminData={}) => {
  let i = 0;
  let data = ``;
...
const updatePassword = async (conditions, password) => {
  let j = 0;
  let where = ``;
  for (const [key, value] of Object.entries(conditions)) {
    if (j > 0) where += ` AND `;
